...
When user runs =touch foo= to create a file this command (run in user space) performs a system call to the kernel.
Kernel on the other hand performs the appropriate filesystem call (depending on the filesystem) to create the file.

I think we can say that user space works on "virtual filesystem".

Filesystem in User Space allows filesystems to be implemented by a user program.
During the lecture there was an example with sending an e-mail each time a file is created. Code for sending e-mails is in the user space.
When we =touch foo= a system call to the kernel is being called. Kernel works on =FUSE= which is able to "callback" code in the user space.

=sshfs= is an example of the FUSE filesystem. We can run commands on the local machine but these are being sent to the remote machine, run there and the output is returned back.
It allows local programs to see files on the remote machine as if they were local.
...
How to remove distinguish what is argument? For example how can we remove file called =-i=? This will be treated as an argument, not filename.
We can mark something as *not* a flag by using double dash. If we use =--= it means "do not interpret anything following this".
Going back to the example we can remove file called =-i= by using =rm -- -i=.
...
Just check the website: https://missing.csail.mit.edu/2020/qa/
